...
export default function (pi: ExtensionAPI) {
  ...
  const backendNames = enabledBackendNames(settings);
  const spawnParameterDescriptions =
    buildSubagentSpawnParameterDescriptions(backendNames);
  ...
  const getRuntime = () =>
    (runtime ??= createSubagentRuntime({ enabledBackends: backendNames }));
  ...
  pi.registerTool({
    ...
    description: buildSubagentSpawnToolDescription(backendNames),
    promptSnippet: buildSubagentSpawnPromptSnippet(backendNames),
    promptGuidelines: buildSubagentSpawnPromptGuidelines(backendNames),
    parameters: Type.Object({
      prompt: Type.String({
        description: spawnParameterDescriptions.prompt,
      }),
      name: Type.String({
        description: spawnParameterDescriptions.name,
      }),
      harness: StringEnum(backendNames, {
        description: spawnParameterDescriptions.harness,
      }),
      working_dir: Type.Optional(
        Type.String({
          description: spawnParameterDescriptions.workingDir,
        }),
      ),
      model: Type.Optional(
        Type.String({
          description: spawnParameterDescriptions.model,
        }),
      ),
      reasoning_effort: Type.Optional(
        StringEnum(REASONING_EFFORTS, {
          description: spawnParameterDescriptions.reasoningEffort,
        }),
      ),
    }),
