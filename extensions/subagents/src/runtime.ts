...
export interface SubagentRuntimeOptions {
  readonly enabledBackends?: readonly BackendName[];
}

const createBackendRegistryLive = (options: SubagentRuntimeOptions) =>
  Layer.sync(BackendRegistry, () => {
    ...
    const enabledNames = new Set(options.enabledBackends ?? BACKEND_NAMES);
    const enabled = backends.filter((backend) =>
      enabledNames.has(backend.name),
    );
    return new Map<BackendName, SubagentBackend>(
      enabled.map((backend) => [backend.name, backend]),
    );
  });

export function createSubagentRuntime(options: SubagentRuntimeOptions = {}) {
  const appLayer = SubagentManagerLive.pipe(
    Layer.provide(createBackendRegistryLive(options)),
  );
