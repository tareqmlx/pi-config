...
Never silently substitute a different harness or model provider when the user names a harness.

When the user requests a Claude Code subagent but `claude` is absent from the `subagent_spawn` harness options:

1. Read `~/.pi/agent/settings.json` and check `enableClaudeSubagent`.
2. If it is `false`, explain that Claude Code subagents are installed but disabled in Pi settings.
3. Tell the user to set `"enableClaudeSubagent": true`, then run `/reload` or restart Pi.
4. Do not use Pi with an OpenCode-hosted Claude model as a substitute unless the user explicitly requests that alternative.

If Claude is enabled but unavailable, check that Claude Code is installed and authenticated before reporting the specific failure.
